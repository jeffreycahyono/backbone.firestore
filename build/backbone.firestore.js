(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["Backbone.firestore"] = factory(require("underscore"), require("backbone"), require("firebase"));
	else
		root["Backbone.firestore"] = factory(root["_"], root["Backbone"], root["firebase"]);
})(typeof self !== 'undefined' ? self : this, function(__WEBPACK_EXTERNAL_MODULE_21__, __WEBPACK_EXTERNAL_MODULE_23__, __WEBPACK_EXTERNAL_MODULE_81__) {
 ...
 */
function getFirestore(model) {
  return (0, _underscore.result)(model, 'firestore') || (0, _underscore.result)(model.collection, 'firestore');
}
...
exports.FirestoreAdapter = exports.deleteCollection = exports.getModelDocRef = exports.unsubscribeUpdate = undefined;

var _backbone = __webpack_require__(23);

var _backbone2 = _interopRequireDefault(_backbone);

var _FirestoreAdapter = __webpack_require__(40);

var _sync = __webpack_require__(82);

var _utils = __webpack_require__(22);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_backbone2.default.FirestoreAdapter = _FirestoreAdapter.FirestoreAdapter;
...
function getSyncMethod(model) {
  ...
  var hasFirestore = (0, _utils.getFirestore)(model);

  return !forceAjaxSync && hasFirestore ? _sync.sync : ajaxSync;
}
...
exports.unsubscribeUpdate = _FirestoreAdapter.unsubscribeUpdate;
exports.getModelDocRef = _FirestoreAdapter.getModelDocRef;
exports.deleteCollection = _FirestoreAdapter.deleteCollection;
exports.FirestoreAdapter = _FirestoreAdapter.FirestoreAdapter;
...
exports.FirestoreAdapter = exports.deleteCollection = exports.getModelDocRef = exports.querySnapshotToArray = exports.unsubscribeUpdate = undefined;
...
var docToJSON = function docToJSON(docRef) {
  return docRef && (0, _underscore.extend)({ id: docRef.id }, docRef.data()) || {};
};

var querySnapshotToArray = function querySnapshotToArray(querySnapshot) {
  var resp = [];
...
var getModelDocRef = function getModelDocRef(model) {
  var adapter = (0, _utils.getFirestore)(model);
  if (!model.id) {
...
var unsubscribeUpdate = function unsubscribeUpdate(modelOrCollection) {
  var unsubscribe = getFirestoreProp(modelOrCollection, 'unsubscribe');
  if (unsubscribe) {
    unsubscribe();
 ...
 */
var subscribeModelUpdate = function subscribeModelUpdate(model) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  return new _promise2.default(function (resolve, reject) {
    options = (0, _underscore.extend)({ parse: true }, options);
    if (isSubscribed(model)) {
      resolve(false);
      return;
    }
    var docRef = getModelDocRef(model);
    var handleSnapshot = function handleSnapshot(doc) {
      model.trigger('firestore:snapshot:model', model, doc);
      if (!options.skipFirstUpdate) {
        if (doc.exists) {
          var resp = docToJSON(doc);
    ...
    var unsubscribe = docRef.onSnapshot(handleSnapshot, handleError);
    setFirestoreProp(model, 'unsubscribe', unsubscribe);
    model.once('destroy remove change:id', function () {
 ...
 * @returns {void}
 */
var subscribeCollectionUpdate = function subscribeCollectionUpdate(collection, options) {
  return new _promise2.default(function (resolve, reject) {
    if (isSubscribed(collection)) {
      unsubscribeUpdate(collection);
    }
    var adapter = collection.firestore;
    var query = adapter.getQuery(options);
    var handleSnapshot = function handleSnapshot(querySnapshot) {
      collection.trigger('firestore:snapshot:collection', collection, querySnapshot);
      if (!options.skipFirstUpdate) {
        var resp = querySnapshotToArray(querySnapshot),
    ...
    var unsubscribe = query.onSnapshot(handleSnapshot, handleError);
...
function deleteCollection(collection) {
  var batchSize = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 16;

  var collectionRef = (0, _utils.getFirestore)(collection).getCollectionRef();
...
var FirestoreAdapter = function () {
   ...
   */
  function FirestoreAdapter(collectionName) {
    (0, _classCallCheck3.default)(this, FirestoreAdapter);
  ...
  (0, _createClass3.default)(FirestoreAdapter, [{
    key: 'getCollectionRef',
    value: function getCollectionRef() {
    ...
    value: function save(model) {
      var firestoreOptions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      var docRef = !model.id && model.id !== 0 ? this.getCollectionRef().doc() : this.getCollectionRef().doc(model.id),
          data = (0, _underscore.omit)(model.toJSON(), 'id');
      return docRef.set(data, firestoreOptions).then(function () {
        return {
          id: docRef.id
        };
      });
    ...
    value: function find(model) {
      ...
      if (options.subscriptionEnabled && !isSubscribed(model)) {
        promise = subscribeModelUpdate(model, (0, _underscore.extend)({ skipFirstUpdate: true }, options));
      } else {
        var docRef = this.getCollectionRef().doc(model.id);
        promise = docRef.get();
      }
      return promise.then(function (doc) {
        return doc.exists ? docToJSON(doc) : false;
      });
    }
  }, {
    key: 'getQuery',
    value: function getQuery(options) {
      var queryFn = options.firestoreQuery;
    ...
    value: function findAll(collection) {
      ...
      if (options.subscriptionEnabled) {
        promise = subscribeCollectionUpdate(collection, (0, _underscore.extend)({ skipFirstUpdate: true }, options));
      } else {
...
exports.unsubscribeUpdate = unsubscribeUpdate;
exports.querySnapshotToArray = querySnapshotToArray;
exports.getModelDocRef = getModelDocRef;
...
function sync(method, model) {
  ...
  var firestore = (0, _utils.getFirestore)(model);
  var promise = void 0;

  switch (method) {
    case 'read':
      promise = (0, _underscore.isUndefined)(model.id) ? firestore.findAll(model, options) : firestore.find(model, options);
      break;
    case 'create':
